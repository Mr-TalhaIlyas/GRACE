...
def calculate_epoch_level_metrics_extended(
    ...
    for modality in modalities:
        ...
        prebs_pr = hysteresis_thresholding(probs_pr, hysteresis_high, hysteresis_low, only_pos_probs=True)
        prebs_pr = apply_temporal_smoothing_preds(prebs_pr, smoothing_window)
    ...
    for modality in modalities:
        ...
        preds_for_kappa = hysteresis_thresholding(probs_for_kappa, hysteresis_high, hysteresis_low, only_pos_probs=True)
    ...
    fusion_preds = hysteresis_thresholding(all_probs[fusion_mod], hysteresis_high, hysteresis_low, only_pos_probs=True)
    fusion_preds = apply_temporal_smoothing_preds(fusion_preds, smoothing_window)

    for modality in modalities:
        if modality == fusion_mod:
            continue
        # Predictions for other modality
        other_preds = hysteresis_thresholding(all_probs[modality], hysteresis_high, hysteresis_low, only_pos_probs=True)
        other_preds = apply_temporal_smoothing_preds(other_preds, smoothing_window)
    ...
    delong_dict = {}
    fusion_probs_sm = apply_temporal_smoothing_probs(all_probs[fusion_mod], smoothing_window)

    for modality in modalities:
        if modality == fusion_mod:
            continue
        other_probs_sm = apply_temporal_smoothing_probs(all_probs[modality], smoothing_window)
