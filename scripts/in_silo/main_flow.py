...
from in_silo.models.flow_model import SlowFast
...
from in_silo.utils.flow_trainer import Trainer, Evaluator
...
EXP_NAME = 'sahzu_flow_in_silo_exp'  # config['experiment_name'] or 'Pose_Exp
...
model = SlowFast(num_classes=len(config['super_classes']),
                 sup_classes=len(config['super_classes']))
model.to(DEVICE)
...
for epoch in range(config['epochs']):
    ...
    if (epoch + 1) % config['val_every'] == 0:
        ...
        if fusion_f1 > best_val_fusion:
            ...
            cprint(f"-> Saving best checkpoint at epoch {epoch+1} with F1-score {fusion_f1:.4f}", 'green')
    # —————— LOG METRICS ——————
    if config['LOG_WANDB']:
        log_dict = {
    ...
    cprint(f"\nEpoch {epoch+1:02d}/{config['epochs']} — "
            ...
            f"Train Fusion F1-score: {train_accs['flow']:.4f} | "
            f"Val Fusion F1-score: {val_f1.get('flow', -1):.4f} | "
